...
public class Student {
	...
	private ModuleRecord [] moduleRecords = new ModuleRecord [13]; //^^
	...
	public boolean checkModuleSelection()
	{
	    ...
	    //check that there is at least one project module, there isn't too many level 6 credits, there is enough total credits and enough credits in each semester
	    return !((sumOtherCredits > 0) || !projectModule || (sumLevel6Credits > MAXIMUM_LEVEL_6_CREDITS) ||
	             ...
	             || sem1Credits < 50 || sem1Credits > 70 || sem2Credits < 50 || sem2Credits > 70
	             || sem3Credits < 50 || sem3Credits > 70);
	...
	{
	    int i = findModuleIndex(m);
	    moduleRecords[i].recordMark(mark); //find the module and record the mark
	}
	
	//calculate the student's overall average mark - this works out the weighted average which you can assume is correct
	...
	{
	    if (checkModuleSelection() && checkAllModulesPassed()) //all modules chosen are valid and no modules have been failed
	    {
	    	if(getAverage() >= MSC_WITH_DISTINCTION_AVERAGE) //clever clogs
	        {
	            return  Qualification.MSC_WITH_DISTINCTION; 
	        }
	        else if(getAverage() >= MSC_WITH_MERIT_AVERAGE) //merit
	        {
	            return Qualification.MSC_WITH_MERIT;
	        }
	        else if(getAverage() >= MSC_AVERAGE) //pass
	        {
	            return Qualification.MSC; 
	        }
	        else //you are the weakest link, goodbye
	        {
	    ...
	    {
	        return Qualification.FAIL; //else fail
	    }
	}
	...
	//Private method to find the next free slot in the array in which a module can be added
